...
parser.add_argument("--outdir", type=str, nargs="?", default="outputs/training_imgs_squarized/",
...
parser.add_argument("--edgedetection", type=str, default="40%",
                    help="strength of edgedetection, percentage strength depends on how much of a complex border your images have ", )
parser.add_argument("--extra_crop", type=int, default=False, help="add extra crop of 10 px on each side", )

# to activate paths in voc
parser.add_argument("--voc", type=int, default=False,
                    help="a command to activate it so it works with visions of chaos paths", )

# to find latent diffusion folder in non voc
parser.add_argument("--latent_diffusion_path", type=str, default="/src/latent-diffusion",
                    help="pass the path to your 'stable-diffusion/src/latent-diffusion' folder", )

opt = parser.parse_args()

if opt.voc:
    visions_path = os.path.expanduser('~\Documents\\visions of chaos')
    latent_diffusion_path = os.path.expanduser(
        '~\AppData\\Roaming\\Visions of Chaos\\Examples\\MachineLearning\\Text To Image\\Stable Diffusion\\src\\latent-diffusion')
    voc = opt.voc
else:
...
def edgeremoval():
    if opt.edgeremoval:
        ...
        subprocess.run(
            ['magick', 'mogrify', '-path', desitiantion2, '-fuzz', edgedetection, '-trim', '-format', 'png', source2])
    else:
        source2 = visions_path + "/tmpsquarize/images/"
        desitiantion2 = visions_path + "/tmpsquarize/resizedimages/"
    ...
    with torch.no_grad():
        with model.ema_scope():
            for image, mask in tqdm(zip(images, masks)):
                ...
                samples_ddim, _ = sampler.sample(S=opt.steps, conditioning=c, batch_size=c.shape[0], shape=shape,
                                                 verbose=False)
